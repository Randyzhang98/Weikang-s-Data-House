...
int partition(int * array, const int & size, const int & order, const int & p)
// Require: The p should be the first elements of the array
{
    ...
    while (left <= right)
    {
        while (array[left] < p && left < size - 1)
        {
            left++;
            // cout << "left ++" << endl;
        }
        while (array[right] >= p && right > 0)
        {
            right--;
            // cout << "right ++" << endl;
        }
...
int deterministic_selection(int * array, const int & size, const int & order)
{
    ...
    index = partition(array, size, order, array[0]);
